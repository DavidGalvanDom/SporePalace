...
(function () {
    ...
    _.extend(bbGrid.View.prototype, Backbone.View.prototype, {
        ...
        showCollection: function (collection, allrows) {
            ...
            if (this.enableTotal) {
                ...
                var cols = _.filter(this.colModel, function (col) { return !col.hidden; });
                ...
                _.each(allrows, function (model) {
                    _.each(cols, function (col) {
                        if (col.total !== undefined) {
                            col.total = parseFloat(col.total) + parseFloat(model.attributes[col.name]);
                        }
                    });
                });
        ...
        renderPage: function (options) {
            ...
            this.showCollection(this.collection.models.slice(interval.s, interval.e), this.collection.models);
    ...
    _.extend(bbGrid.RowView.prototype, Backbone.View.prototype, {
        ...
        template: _.template(
            ...
            <%} if (isActionEnable) {%>\
                <td style="overflow:visible;">\
                    <div class="btn-group">\
                        <button class="btn btn-outline btn-default dropdown-toggle" data-toggle="dropdown" data-rowid=""> Accion <span class="caret"></span></button>\
                        <ul class="dropdown-menu" data-modelid="<%=modelId%>">\
                            <% if (isEditar === undefined || isEditar === true) {%>\
                            <li><a href="#" class="accrowEdit"><i class="fa fa-edit"></i>  Editar</a></li>\
                            ...
                            <% if (isPicture === true) {%>\
                                <li class="divider"></li>\
                                <li><a href="#" class="accrowImage"><i class="fa fa-picture"></i>  Imagen</a></li>\
                                <%}%>\
                            <%} if (isActionPlus) {%>\
                                <li><a href="#" class="accrowCorreo"><i class="fa fa-envelope"></i>  Correo</a></li>\
                            <%}%>\
                        </ul>\
                    </div>\
                </td>\
        ...
        render: function () {
            ...
            html = this.template({
                ...
                isActionEnable: this.view.actionenable,
    ...
    bbGrid.TotalView = function (options){
        Backbone.View.apply(this, [options]);
        this.view = options.view;
        this.cols = options.cols;
    };
    _.extend(bbGrid.TotalView.prototype, Backbone.View.prototype, {
        tagName: 'tr',
        ...
        template: _.template(
            ...
            <%} if (isContainSubgrid) {%>\
                <td style="width:15px"/>\
            <%} _.each(cols, function (col) {%>\
                <td <%if (col.width) {%>style="width:<%=col.width%>"<%}%> >\
                <%if (col.total !== undefined) {%>  \
                   <%=col.total %>\
                <% }%>  </td> }\
            <%})%>', null, templateSettings),
    ...
    bbGrid.TotalView.extend = Backbone.View.extend;
    ...
    _.extend(bbGrid.FilterView.prototype, Backbone.View.prototype, {
        ...
        onFilter: function () {            
            ...
            _.each($('*[name=filter]', this.$el), function (el) {
                ...
                text = $.trim($(el).val());
                self.view.filterOptions[name] = text;
            });
